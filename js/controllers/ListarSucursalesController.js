...
angular.module('MetronicApp').controller('ListarSucursalesController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
...
$rootScope.$state.go('login');
        ...
        grid.init({
            ...
            dataTable: { 
                ...
                "ajax": {
                    url: "database/SucursalGet.php?action_type=list", // ajax source
                    dataSrc: "data"
                },
                            ...
                            return '<div class="btn-group">' +
                                        '<button class="btn btn-xs btn-warning dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Acciones <i class="fa fa-angle-down"></i></button>' +
                                         '<ul class="dropdown-menu pull-right" role="menu">' +
                                            '<li><a href=#/private/registrar_sucursal/' + data['idsucursales'] + '><i class="fa fa-pencil"></i> Editar sucursal</a></li>' +
                                            '<li><a href="javascript:;" data-id="' + data['idsucursales'] + '" class="mt-sweetalert delete"><i class="fa fa-times"></i> Eliminar sucursal</a></li>' +
                                        '</ul>' +
                                    '</div>';               
                          }
                        }
		...
		/*******************************/
		$('#datatable_sucursales tbody').on( 'click', 'a.mt-sweetalert', function () {
			...
			var rowToDelete = $(this).parents('tr');
					...
					function(){
							...
							$.ajax({
								dataType:'JSON',
								type: 'POST',
								url: 'database/SucursalGet.php?action_type=delete&id='+data['idsucursales'],
								success:function(data){
										...
										switch(data.code){
											...
											case "1451"	:	swal("ERROR", "El registro no se puede eliminar ya que está asociado a otro documento. Para poder eliminar la sucursal, deberá todos los registros asociados a este registro.", "error");
															break;
