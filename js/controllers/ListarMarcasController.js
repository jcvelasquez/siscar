...
angular.module('MetronicApp').controller('ListarMarcasController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
...
$rootScope.$state.go('login');
        ...
        grid.init({
            ...
            dataTable: { 
                ...
                "ajax": {
                    url: "database/MarcasVehiculosGet.php?action_type=list", // ajax source
                    dataSrc: "data"
                },
                
                columns: [
                        { data : "idmarcas_vehiculos" },
                        { data : "marca_vehiculo" },
                        {
                            "mData": null,
                            "bSortable": false,
                            ...
                            return '<div class="btn-group">' +
                                        '<button class="btn btn-xs btn-warning dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Acciones <i class="fa fa-angle-down"></i></button>' +
                                         '<ul class="dropdown-menu pull-right" role="menu">' +
                                            '<li><a href=#/private/registrar_marca/' + data['idmarcas_vehiculos'] + '><i class="fa fa-pencil"></i> Editar marca</a></li>' +
                                            '<li><a href="javascript:;" data-id="' + data['idmarcas_vehiculos'] + '" class="mt-sweetalert delete"><i class="fa fa-times"></i> Eliminar marca</a></li>' +
                                        '</ul>' +
                                    '</div>';               
                          }
                        }
		...
		/*******************************/
		$('#datatable_marcas tbody').on( 'click', 'a.mt-sweetalert', function () {
			...
			var rowToDelete = $(this).parents('tr');
					...
					function(){
							...
							$.ajax({
								dataType:'JSON',
								type: 'POST',
								url: 'database/MarcasVehiculosGet.php?action_type=delete&id='+data['idmarcas_vehiculos'],
