...
angular.module('MetronicApp').controller('ListarRolesController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
...
$rootScope.$state.go('login');
        ...
        grid.init({
            ...
            dataTable: { 
                ...
                "ajax": {
                    url: "database/RolesGet.php?action_type=list", // ajax source
                    dataSrc: "data",
					type: "POST",
                ...
                columns: [
                        { data : "idroles" },
						{
							  ...
							  "mRender": function(data, type, full) {
										return '<a href=#/private/registrar_rol/' + data['idroles'] + '>' + data['cod_rol'] + '</a>';
                            ...
                            return '<div class="btn-group">' +
                                        '<button class="btn btn-xs btn-warning dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Acciones <i class="fa fa-angle-down"></i></button>' +
                                         '<ul class="dropdown-menu pull-right" role="menu">' +
                                            '<li><a href=#/private/registrar_rol/' + data['idroles'] + '><i class="icon-docs"></i> Editar</a></li>' +
                                            '<li><a href="javascript:;" data-id="' + data['idroles'] + '" class="mt-sweetalert delete"><i class="fa fa-times"></i> Eliminar</a></li>' +
                                        '</ul>' +
                                    '</div>';               
                          }
                        }
		...
		/*******************************/
		$('#datatable_roles tbody').on( 'click', 'a.mt-sweetalert', function () {
		...
		var confirm_delete = swal({
					...
					function(){
							...
							$.ajax({
								dataType:'JSON',
								type: 'POST',
								url: 'database/RolesGet.php?action_type=delete&id='+data['idroles'],
