...
angular.module('MetronicApp').controller('ListarMedidasCombustibleController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
    $scope.$on('$viewContentLoaded', function() {   
        ...
        grid.init({
            src: $("#datatable_medida_combustible"),
            ...
            dataTable: { 
                ...
                "ajax": {
                    url: "database/MedidaCombustibleGet.php?action_type=list", // ajax source
                    dataSrc: "data"
                },
                
                columns: [
                        { data : "idmedida_combustible" },
                        { data : "medida_combustible" },
                        {
                            ...
                            "mRender": function(data, type, full) {
                                ...
                                if(data['estado_medida_combustible'] == 1) 
                            ...
                            return '<div class="btn-group">' +
                                        '<button class="btn btn-xs btn-warning dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Acciones <i class="fa fa-angle-down"></i></button>' +
                                         '<ul class="dropdown-menu pull-right" role="menu">' +
                                            '<li><a href=#/private/registrar_medida_combustible/' + data['idmedida_combustible'] + '><i class="icon-docs"></i> Editar</a></li>' +
                                            '<li><a href="javascript:;" data-id="' + data['idmedida_combustible'] + '" class="mt-sweetalert delete"><i class="icon-docs"></i> Eliminar</a></li>' +
                                        '</ul>' +
                                    '</div>';               
                          }
                        }
		...
		/*******************************/
		$('#datatable_medida_combustible tbody').on( 'click', 'a.mt-sweetalert', function () {
			...
			var rowToDelete = $(this).parents('tr');
					...
					function(){
							...
							$.ajax({
								dataType:'JSON',
								type: 'POST',
								url: 'database/MedidaCombustibleGet.php?action_type=delete&id='+data['idmedida_combustible'],
