...
angular.module('MetronicApp').controller('RegistrarTipoCombustibleController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
				...
				if (idItem != '' && idItem != 'nuevo') {
					...
					//CAMPOS DE EDICION							
					 $.ajax({
						dataType:'JSON',
						type: 'POST',
						url: 'database/TipoCombustibleGet.php?action_type=edit&id='+idItem,
						success:function(data){
							
							//INICIO DE PERSONALIZACION DE CAMPOS
							$('#tipo_combustible').val(data.tipo_combustible);
							$("#estado_tipo_combustible").val(data.estado_tipo_combustible).change();
					...
					$("#estado_tipo_combustible").val(1).change().selectpicker('refresh');
				 ...
				 form1.validate({
						...
						submitHandler: function (form) {
								...
								$.ajax({
										dataType:'JSON',
										type: 'POST',
										url: 'database/TipoCombustibleGet.php',
										data: formdata,
										success:function(data){
											...
											$rootScope.$state.go('private.listar_tipos_combustibles');
