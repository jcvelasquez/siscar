...
angular.module('MetronicApp').controller('ListarTiposCombustiblesController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
...
$rootScope.$state.go('login');
        ...
        grid.init({
            src: $("#datatable_tipos_combustibles"),
            ...
            dataTable: { 
                ...
                "ajax": {
                    url: "database/TipoCombustibleGet.php?action_type=list", // ajax source
                    dataSrc: "data",
					type: "POST",
                ...
                columns: [
                        ...
                        { data : "tipo_combustible" },
						{
                            ...
                            "mRender": function(data, type, full) {
                                ...
                                if(data['estado_tipo_combustible'] == 1) 
                            ...
                            return '<div class="btn-group">' +
                                        '<button class="btn btn-xs btn-warning dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Acciones <i class="fa fa-angle-down"></i></button>' +
                                         '<ul class="dropdown-menu pull-right" role="menu">' +
                                            '<li><a href=#/private/registrar_tipo_combustible/' + data['idtipo_combustible'] + '><i class="icon-docs"></i> Editar</a></li>' +
                                            '<li><a href="javascript:;" data-id="' + data['idtipo_combustible'] + '" class="mt-sweetalert delete"><i class="fa fa-times"></i> Eliminar</a></li>' +
                                        '</ul>' +
                                    '</div>';               
                          }
                        }
		...
		/*******************************/
		$('#datatable_tipos_combustibles tbody').on( 'click', 'a.mt-sweetalert', function () {
			...
			var confirm_delete = swal({
						...
						function(){
							...
							$.ajax({
								dataType:'JSON',
								type: 'POST',
								url: 'database/TipoCombustibleGet.php?action_type=delete&id='+data['idtipo_combustible'],
