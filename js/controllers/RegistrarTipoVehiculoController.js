...
angular.module('MetronicApp').controller('RegistrarTipoVehiculoController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
				...
				if (idItem != '' && idItem != 'nuevo') {
					...
					//CAMPOS DE EDICION							
					 $.ajax({
						dataType:'JSON',
						type: 'POST',
						url: 'database/TipoVehiculoGet.php?action_type=edit&id='+idItem,
						success:function(data){
							console.log(data);
							//INICIO DE PERSONALIZACION DE CAMPOS
							$('#tipo_vehiculo').val(data.tipo_vehiculo);
							$("#estado_tipo").val(data.estado_tipo).change();
					...
					$("#estado_tipo").val(1).change().selectpicker('refresh');
				 ...
				 form1.validate({
						...
						submitHandler: function (form) {
								...
								$.ajax({
										dataType:'JSON',
										type: 'POST',
										url: 'database/TipoVehiculoGet.php',
										data: formdata,
										success:function(data){
											...
											$rootScope.$state.go('private.listar_tipos_vehiculos');
