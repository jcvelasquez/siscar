...
/* Setup general page controller */
angular.module('MetronicApp').controller('ListarAlertasController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
...
$rootScope.$state.go('login');
        ...
        grid.init({
            ...
            dataTable: { 
                ...
                "ajax": {
                    url: "database/MantenimientoAlertasGet.php?action_type=list", // ajax source
                    dataSrc: "data"
                },
                            ...
                            return '<div class="btn-group">' +
                                        '<button class="btn btn-xs btn-warning dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> <i class="fa fa-gear"></i>  Acciones <i class="fa fa-angle-down"></i></button>' +
                                         '<ul class="dropdown-menu pull-right" role="menu">' +
                                            '<li><a href=#/private/registrar_alerta/' + data['idmantenimiento_alertas'] + '><i class="fa fa-pencil"></i> Editar alerta</a></li>' +
                                            '<li><a href="javascript:;" data-id="' + data['idmantenimiento_alertas'] + '" class="mt-sweetalert delete"><i class="fa fa-times"></i>  Eliminar alerta</a></li>' +
                                        '</ul>' +
                                    '</div>';               
                          }
                        }
		...
		/*******************************/
		$('#datatable_alertas tbody').on( 'click', 'a.mt-sweetalert', function () {
			...
			var rowToDelete = $(this).parents('tr');
					...
					function(){
							...
							$.ajax({
								dataType:'JSON',
								type: 'POST',
								url: 'database/MantenimientoAlertasGet.php?action_type=delete&id='+data['idmantenimiento_alertas'],
