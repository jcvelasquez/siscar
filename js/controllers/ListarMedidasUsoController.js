...
angular.module('MetronicApp').controller('ListarMedidasUsoController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
...
$rootScope.$state.go('login');
        ...
        grid.init({
            ...
            dataTable: { 
                ...
                "ajax": {
                    url: "database/MedidaUsoGet.php?action_type=list", // ajax source
                    dataSrc: "data"
                },
                            ...
                            return '<div class="btn-group">' +
                                        '<button class="btn btn-xs btn-warning dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Acciones <i class="fa fa-angle-down"></i></button>' +
                                         '<ul class="dropdown-menu pull-right" role="menu">' +
                                            '<li><a href=#/private/registrar_medida_uso/' + data['id_medida_uso'] + '><i class="icon-docs"></i> Editar</a></li>' +
                                            '<li><a href="javascript:;" data-id="' + data['id_medida_uso'] + '" class="mt-sweetalert delete"><i class="icon-docs"></i> Eliminar</a></li>' +
                                        '</ul>' +
                                    '</div>';               
                          }
                        }
		...
		/*******************************/
		$('#datatable_medidas_uso tbody').on( 'click', 'a.mt-sweetalert', function () {
			...
			var rowToDelete = $(this).parents('tr');
					...
					function(){
							...
							$.ajax({
								dataType:'JSON',
								type: 'POST',
								url: 'database/MedidaUsoGet.php?action_type=delete&id='+data['id_medida_uso'],
