...
angular.module('MetronicApp').controller('ListarEscalasMovilidadController', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, settings) {
...
$rootScope.$state.go('login');
        ...
        grid.init({
            src: $("#datatable_escalas_movilidad"),
            ...
            dataTable: { 
                ...
                "ajax": {
                    url: "database/ServiciosEscalasMovilidadGet.php?action_type=list", // ajax source
                    dataSrc: "data",
					type: "POST",
                },
                columns: [
								
                        { data : "idservicios_escala_movilidades" },
						{
							  "mData": null,
							  "bSortable": false,
							  "mRender": function(data, type, full) {
										return '<a href=#/private/registrar_escala_movilidad/' + data['idservicios_escala_movilidades'] + '>' + data['destino'] + '</a>';
                            ...
                            return '<div class="btn-group">' +
                                        '<button class="btn btn-xs btn-warning dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Acciones <i class="fa fa-angle-down"></i></button>' +
                                         '<ul class="dropdown-menu pull-right" role="menu">' +
                                            '<li><a href=#/private/registrar_escala_movilidad/' + data['idservicios_escala_movilidades'] + '><i class="fa fa-pencil"></i> Editar escala</a></li>' +
                                            '<li><a href="javascript:;" data-id="' + data['idservicios_escala_movilidades'] + '" class="mt-sweetalert delete"><i class="fa fa-times"></i> Eliminar escala</a></li>' +
                                        '</ul>' +
                                    '</div>';               
                          }
                        }
		...
		/*******************************/
		$('#datatable_escalas_movilidad tbody').on( 'click', 'a.mt-sweetalert', function () {
		...
		var confirm_delete = swal({
					...
					function(){
							...
							$.ajax({
								dataType:'JSON',
								type: 'POST',
								url: 'database/ServiciosEscalasMovilidadGet.php?action_type=delete&id='+data['idservicios_escala_movilidades'],
